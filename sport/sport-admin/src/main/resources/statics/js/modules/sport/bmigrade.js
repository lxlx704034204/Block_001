$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sport/bmigrade/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.bmiGrade.id == null ? "sport/bmigrade/save" : "sport/bmigrade/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.bmiGrade),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sport/bmigrade/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sport/bmigrade/info/"+id, function(r){
                vm.bmiGrade = r.bmiGrade;
            });
		},
