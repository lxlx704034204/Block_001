$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '学校所在市', name: 'city', index: 'city', width: 80 }, 			
			{ label: '学校详细地址', name: 'address', index: 'address', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.school.id == null ? "sport/school/save" : "sport/school/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sport/school/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sport/school/info/"+id, function(r){
                vm.school = r.school;
            });
		},
